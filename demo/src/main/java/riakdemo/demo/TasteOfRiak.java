...
public class TasteOfRiak {
	...
	public static void main(String[] args) {
		try {
			System.out.println("Taste of Riak!!!");
			...
			// In the new Java client, instead of buckets you interact with
			// Namespace
			// objects, which consist of a bucket AND a bucket type; if you
			// don't
			// supply a bucket type, "default" is used; the Namespace below will
			// set
			// only a bucket, without supplying a bucket type
			Namespace quotesBucket = new Namespace("quotes");

			// With our Namespace object in hand, we can create a Location
			// object,
			...
			StoreValue storeOp = new StoreValue.Builder(quoteObject)
					.withLocation(quoteObjectLocation).build();
			System.out.println("StoreValue operation created");

			// And now we can use our setUpCluster() function to create a
			// cluster
			...
			System.out
					.println("Object storage operation successfully completed");
			...
			FetchValue fetchOp = new FetchValue.Builder(quoteObjectLocation)
					.build();
			RiakObject fetchedObject = client.execute(fetchOp).getValue(
					RiakObject.class);
			assert (fetchedObject.getValue().equals(quoteObject.getValue()));
			System.out
					.println("Success! The object we created and the object we fetched have the same value");

			// Now update the fetched object
			fetchedObject.setValue(BinaryValue
					.create("You can be my wingman any time."));
			...
			StoreValue storeBookOp = new StoreValue.Builder(mobyDick)
					.withLocation(mobyDickLocation).build();
			...
			FetchValue fetchMobyDickOp = new FetchValue.Builder(
					mobyDickLocation).build();
			Book fetchedBook = client.execute(fetchMobyDickOp).getValue(
					Book.class);
			...
			// Now that we're all finished, we should shut our cluster object
			// down
			cluster.shutdown();
